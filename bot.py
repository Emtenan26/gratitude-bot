...
def generate_morning_message() -> str:
    return ask_claude(
        ...
        "1. تحية صباحية دافئة\n"
        "2. نعمة واحدة من نعم الله (مثل: نعمة البصر، نعمة الأمان، نعمة الصحة...)\n"
...
async def send_morning_messages(context: ContextTypes.DEFAULT_TYPE):
    """يُرسل الرسالة الصباحية لكل المستخدمين المشتركين."""
...
async def send_reminders(context: ContextTypes.DEFAULT_TYPE):
    """يُرسل تذكيراً لمن لم يرد بعد 3 ساعات من رسالة المساء."""
    users = db.get_all_users()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db.user_exists(user_id):
        await update.message.reply_text(
            "أهلاً! اضغط /start للاشتراك في بوت الامتنان 🌸"
        )
        return
